...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	if v := ctx.Value(ctxkey.ClientRequestID); v != nil {
		...
		logger.LegacyPrintf("service.gateway", "[DEBUG] ClientRequestID from context: %s", clientReqID)
	}
	// 如果没有 ClientRequestID，生成一个新的 UUID（兜底）
	if clientReqID == "" {
		clientReqID = uuid.New().String()
		logger.LegacyPrintf("service.gateway", "[ERROR] ClientRequestID not found in context, generated new UUID: %s", clientReqID)
	}
...
func (s *GatewayService) RecordRequest(ctx context.Context, input *RecordRequestInput) error {
	...
	if clientReqID == "" {
		clientReqID = uuid.New().String()
		logger.LegacyPrintf("service.gateway", "Warning: Both ClientRequestID and RequestID are empty for request log, generated new UUID: %s", clientReqID)
	}
	...
	if err := s.requestLogRepo.Create(ctx, requestLog); err != nil {
		logger.LegacyPrintf("service.gateway", "Failed to create request log: %v", err)
		return err
	}
