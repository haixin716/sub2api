...
func (_q *APIKeyQuery) QueryRequestLogs() *RequestLogQuery {
...
func (_q *APIKeyQuery) WithRequestLogs(opts ...func(*RequestLogQuery)) *APIKeyQuery {
	query := (&RequestLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withRequestLogs; query != nil {
		if err := _q.loadRequestLogs(ctx, query, nodes,
			func(n *APIKey) { n.Edges.RequestLogs = []*RequestLog{} },
			func(n *APIKey, e *RequestLog) { n.Edges.RequestLogs = append(n.Edges.RequestLogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadRequestLogs(ctx context.Context, query *RequestLogQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *RequestLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(requestlog.FieldAPIKeyID)
	}
	query.Where(predicate.RequestLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.RequestLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
