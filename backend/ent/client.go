...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.Proxy, c.RedeemCode, c.RequestLog, c.Setting, c.UsageCleanupTask, c.UsageLog,
...
// QueryRequestLogs queries the request_logs edge of a Account.
func (c *AccountClient) QueryRequestLogs(_m *Account) *RequestLogQuery {
	query := (&RequestLogClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryRequestLogs queries the request_logs edge of a Group.
func (c *GroupClient) QueryRequestLogs(_m *Group) *RequestLogQuery {
	query := (&RequestLogClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(group.Table, group.FieldID, id),
			sqlgraph.To(requestlog.Table, requestlog.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, group.RequestLogsTable, group.RequestLogsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryAccount queries the account edge of a RequestLog.
func (c *RequestLogClient) QueryAccount(_m *RequestLog) *AccountQuery {
	query := (&AccountClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryGroup queries the group edge of a RequestLog.
func (c *RequestLogClient) QueryGroup(_m *RequestLog) *GroupQuery {
	query := (&GroupClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		...
		RedeemCode, RequestLog, Setting, UsageCleanupTask, UsageLog, User,
