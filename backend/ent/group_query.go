...
func (_q *GroupQuery) QueryRequestLogs() *RequestLogQuery {
...
func (_q *GroupQuery) WithRequestLogs(opts ...func(*RequestLogQuery)) *GroupQuery {
	query := (&RequestLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withRequestLogs; query != nil {
		if err := _q.loadRequestLogs(ctx, query, nodes,
			func(n *Group) { n.Edges.RequestLogs = []*RequestLog{} },
			func(n *Group, e *RequestLog) { n.Edges.RequestLogs = append(n.Edges.RequestLogs, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadRequestLogs(ctx context.Context, query *RequestLogQuery, nodes []*Group, init func(*Group), assign func(*Group, *RequestLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(requestlog.FieldGroupID)
	}
	query.Where(predicate.RequestLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.RequestLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
