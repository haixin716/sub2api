-- 修复唯一约束问题
-- 为 usage_logs 和 request_logs 添加各自的稳定请求键唯一约束

BEGIN;

-- 1. usage_logs 沿用 f_dev_3.0 的 (request_id, api_key_id) 幂等键
-- 注意：如果表中已有重复数据，此操作会失败
-- 可以先查询检查：SELECT request_id, api_key_id, COUNT(*) FROM usage_logs GROUP BY request_id, api_key_id HAVING COUNT(*) > 1;
